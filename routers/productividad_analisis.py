...
def _query_productive_db_via_jdbc(query: str, fecha_desde: str, fecha_hasta: str, legajo: str | None = None) -> list[dict[str, Any]]:
    ...
    if (
        ...
        and "FCREAREG" in normalized_query
        and "COPECREA" in normalized_query
        and "FECHA_LOGISTICA" not in normalized_query
        ...
        "FROM VW_UBICACIONES_DIVISION" in normalized_query
        and "CPASILLO" in normalized_query
        and "CHUECOPA" in normalized_query
        and "CDIVISIO" in normalized_query
    ):
        query_key = "raw_sql_env"
    elif (
        "FROM F132HIST A" in normalized_query
        and "CUBIORIG" in normalized_query
        and "CZONAORI" in normalized_query
        and "QCANTIDA" in normalized_query
    ):
        query_key = "raw_sql_env"
    elif (
        "FROM F132HIST A" in normalized_query
        and "CDESCRIP AS OPERACION" in normalized_query
        and "COUNT(DISTINCT" in normalized_query
        and "SUM(A.QCANTIDA) AS BULTOS" in normalized_query
    ):
        query_key = "raw_sql_env"
    elif (
